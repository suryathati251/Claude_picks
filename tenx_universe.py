"""
tenx_universe.py — broader-market scan universe for the 10x Radar.

Source: S&P 500 + Nasdaq-100 constituents (Wikipedia, snapshot 2026-07-12),
plus a small curated list of high-revenue-growth names outside both indexes.
Tickers already in the app's WATCHLIST are deduped at runtime by app.py.

Only quarterly revenue data (free, Yahoo) + one batch quote (market cap) are
fetched for these names — they do NOT consume the FMP fundamentals budget.

Class-share duplicates (GOOG vs GOOGL, FOX/FOXA, NWS/NWSA) are collapsed to one
listing. Dotted tickers use the dash form (BRK-B, BF-B) that Yahoo/FMP accept.

To refresh this list: re-copy the constituent tables from
https://en.wikipedia.org/wiki/List_of_S%26P_500_companies and
https://en.wikipedia.org/wiki/Nasdaq-100 into _RAW below (ticker|name|sector).
"""
...
TSSI|TSS Inc.|Information Technology
"""

SCAN_UNIVERSE: list[dict] = []
_seen: set[str] = set()
for _line in _RAW.strip().splitlines():
    _tk, _name, _sector = (p.strip() for p in _line.split("|"))
    if _tk in _seen:
        continue
    _seen.add(_tk)
    SCAN_UNIVERSE.append({"ticker": _tk, "name": _name, "sector": _sector})

SCAN_SYMBOLS = [row["ticker"] for row in SCAN_UNIVERSE]
SCAN_NAMES = {row["ticker"]: row["name"] for row in SCAN_UNIVERSE}
SCAN_SECTORS = {row["ticker"]: row["sector"] for row in SCAN_UNIVERSE}
